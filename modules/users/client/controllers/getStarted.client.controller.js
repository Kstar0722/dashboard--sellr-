...
angular.module('users')
  .controller('GetStartedController', ['$scope', '$state', '$stateParams', '$timeout', '$sce', 'storesService', 'Authentication', 'accountsService', '$q', 'toastr', 'Users', '$analytics', 'productsService', 'authenticationService', '$window',
    function ($scope, $state, $stateParams, $timeout, $sce, storesService, Authentication, accountsService, $q, toastr, Users, $analytics, productsService, authenticationService, $window) {
      ...
      $scope.createFacebookUser = function () {
        ...
        authenticationService.getFacebookUserData().then(
          function (facebookData) {
            $scope.data.user.firstName = facebookData.first_name
            $scope.data.user.lastName = facebookData.last_name
            $scope.data.user.email = facebookData.email
            $scope.data.isFacebookUser = true
      ...
      $scope.createUser = function (facebookUser) {
        if (_.isUndefined(facebookUser) && $scope.userForm.$invalid) {
        ...
        if (facebookUser) {
          user.details = {facebookUser: true}
          Users.createUserFacebook(user).then(successCreateUserHandler).catch(errorCreateUserHandler).finally(finallyCreateUserHandler)
        } else {
          Users.create(user).then(successCreateUserHandler).catch(errorCreateUserHandler).finally(finallyCreateUserHandler)
        }

        function successCreateUserHandler (newUser) {
          loadUser(newUser)
          $scope.ui.step = 2
        }
      ...
      $scope.fullscreen = function () {
        return $scope.ui.step === 2 && $scope.ui.storeEditorView || $scope.ui.step === 3 && $scope.ui.addProductsView
      }
