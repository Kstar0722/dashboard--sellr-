...
angular.module('users.admin').controller('AdminPricingController', ['$scope', '$state', '$http', 'Authentication', '$timeout', 'Users', 'Upload', '$sce', 'ImageService', 'constants',
    function ($scope, $state, $http, Authentication, $timeout, Users, Upload, $sce, ImageService, constants) {
      ...
      $scope.addPackage = function (number) {
        ...
        devices[0].qty = Math.round((0.66 * number) * 1) / 1
        devices[1].qty = Math.round((0.33 * number) * 1) / 1
        apps[0].qty = number
        accessories[0].qty = Math.round((0.66 * number) * 1) / 1
        accessories[1].qty = number
        accessories[2].qty = Math.round((0.33 * number) * 1) / 1
        ...
        var packageTotal = (devices[0].price * Math.round((0.66 * number) * 1) / 1) +
          (devices[1].price * Math.round((0.33 * number) * 1) / 1) +
          (apps[0].price * number) +
          (accessories[0].price * Math.round((0.66 * number) * 1) / 1) +
          ((accessories[1].price * number * 1) / 1) +
          (accessories[2].price * Math.round((0.33 * number) * 1) / 1)
