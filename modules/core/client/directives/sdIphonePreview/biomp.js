...
window.bioMp = function (el, options) {
  ...
  var side = {width: 302, height: 889, ratio: (302 / 889)}

  // Options
  options = options || {}
  var url = setOption(options.url, '')
  var view = setOption(options.view, 'front')
  var image = setOption(options.image, '')
  var scale = setOption(options.scale, 0)
  var width = setOption(options.width, 0)
  var height = setOption(options.height, 0)
